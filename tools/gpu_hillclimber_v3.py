"""
GPU-Accelerated Hillclimber v3 -- Liber Primus P21-P54
======================================================
Major change vs v2: HYBRID SCORING -- quadgrams + LP word-slot bonus.

Why:
  v2 used pure quadgram scoring weighted toward LP vocabulary.  After 22M+
  steps the score froze at -132,377.  Root cause: for an OTP-style key,
  changing one position only affects 4 overlapping quadgrams.  Once SA
  reaches "most English-looking" solution the gradient is zero -- because
  LP text is Old English/Cicada register, not generic English.

  Audit also found all 9 confirmed cribs violated word boundaries (spanning
  2-4 separate word slots).  Removing them lets the search explore freely.

What changed:
  1. CONFIRMED_CRIBS = []       -- all false-positive cribs removed
  2. LP word-slot scoring in both SCORE_KERNEL and STEP_KERNEL
       For each word slot (known from '-' word separators), score =
       WORD_WEIGHT * max(0, wl - min_HD_to_any_LP_word_of_that_length)
       This creates gradient toward LP vocabulary at word boundaries.
  3. LP vocabulary from solved pages 0-20 + 55-74 (data-driven)
  4. Warmstart from checkpoint: scatter former crib positions heavily

Usage: python gpu_hillclimber_v3.py <gpu_id>
"""
...
# 65536 = 256 blocks x 256 threads. RTX 2080 Ti has 68 SM x 2048 threads.
# STEP_KERNEL fills ~47% of GPU per launch vs <3% at N_CHAINS=4000 -- 16x more
# chain-steps per second at the same kernel launch rate.
# Keys: 65536 x 14529 x 4 = 3.81 GB; total GPU footprint ~4 GB, safe in 11 GB.
N_CHAINS   = 65_536
SAVE_EVERY = 10_000
OUTFILE    = f'data/gpu_hill_v3_gpu{GPU_ID}.txt'
CHECKPOINT = f'data/gpu_hill_checkpoint_gpu{GPU_ID}_v3.json'
M          = 29
...
print(f'  TTP: {sum(ln for _,_,ln in TTP_CONSTRAINTS)} linked -> {N_INDEPENDENT} independent', flush=True)

# --- NO confirmed cribs -- all prior cribs violated word boundaries ------------
# The audit showed all 9 CONFIRMED_CRIBS in v2 spanned 2-4 word slots each.
# Removed. Word-slot scoring will naturally reward correct LP words at boundaries.
CONFIRMED_CRIBS  = []
FORCED_CRIBS_POS = {}
CRIB_CANON_SET   = set()
print('  Cribs: 0 (all removed -- violated word boundaries)', flush=True)
