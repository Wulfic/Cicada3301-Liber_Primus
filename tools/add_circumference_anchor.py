...
for phrase, start in new_cribs:
    enc = encode(phrase)
    print(f'Adding {len(enc)} positions for {phrase!r} at {start}: {[IDX_TO[v] for v in enc]}')
    added = 0
    for i, v in enumerate(enc):
        ...
        if pos_str not in a['anchors']:
            ...
            if existing != v:
                print(f'  CONFLICT at pos {pos_str}: existing={IDX_TO[existing]}, new={IDX_TO[v]}')
